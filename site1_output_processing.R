...
E_output <- read.csv("output/site1/E_q_all_prob_mn.csv", header=TRUE)
CUFA_150_output <- read.csv("output/site1/CUFA_150_q_all_prob_mn.csv", header=TRUE)
CUFA_nomin_output <- read.csv("output/site1/CUFA_nomin_q_all_prob_mn.csv", header=TRUE)
nrow(E_output)
...
load("output/site1/E_q1_cell_probs.Rdata")
load("output/site1/E_q2_cell_probs.Rdata")
load("output/site1/E_q3_cell_probs.Rdata")
...
load("output/site1/CUFA_150_q3_cell_probs.Rdata")
...
load("output/site1/CUFA_nomin_q3_cell_probs.Rdata")
...
load("output/site1/E_evidence3.Rdata")
...
load("output/site1/CUFA_150_evidence3.Rdata")
...
load("output/site1/CUFA_nomin_evidence3.Rdata")
...
CUFA_150_gis <- data.frame(site1_xy_trim, 
...
write.table(CUFA_150_gis, file = "output/site1/CUFA_150_gis_site1.txt", sep = "\t",  row.names = FALSE)

CUFA_nomin_gis <- data.frame(site1_xy_trim, 
                           existing_prob = E_output[, 2], 
...
write.table(CUFA_nomin_gis, file = "output/site1/CUFA_nomin_gis_site1.txt", sep = "\t",  row.names = FALSE)
...
writeOGR(CUFA_150_gis, "/Users/Morrison/Documents/GIS/Projects/gila bn/shapefiles/site1", "CUFA_150_site1", driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR(CUFA_nomin_gis, "/Users/Morrison/Documents/GIS/Projects/gila bn/shapefiles/site1", "CUFA_nomin_site1", driver="ESRI Shapefile", overwrite_layer=TRUE)
...
E_lengths_site1 <- combine.lengths.E()
CUFA_150_lengths_site1 <- combine.lengths.CUFA.150()
CUFA_nomin_lengths_site1 <- combine.lengths.CUFA.nomin()

# Calculate difference in events
E_lengthsdiff_site1 <- (E_lengths_site1[ ,1] - E_lengths_site1[ ,1])/E_lengths_site1[ ,1] * 100
CUFA_150_lengthsdiff_site1 <- (E_lengths_site1[ ,1] - CUFA_150_lengths_site1[ ,1])/E_lengths_site1[ ,1] * 100
CUFA_nomin_lengthsdiff_site1 <- (E_lengths_site1[ ,1] - CUFA_nomin_lengths_site1[ ,1])/E_lengths_site1[ ,1] * 100

# Apply utility function to calculate percent decrease in probabilities
E_utility_site1 <- utility_ftn(E_lengthsdiff_site1)
CUFA_150_utility_site1 <- utility_ftn(CUFA_150_lengthsdiff_site1)
CUFA_nomin_utility_site1 <- utility_ftn(CUFA_nomin_lengthsdiff_site1)

# Apply utility function decreases to probabilities
E_util_site1 <- E_site1[ ,1] * E_utility_site1
...
# Export as .Rdata
save(combined_site1, file="output/combined_site1.Rdata")
