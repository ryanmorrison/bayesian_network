...
# Site 5
# alldata <- read.table("data/s5_1000_to_2000_trim.txt", header=FALSE, skip=0)
...
s1_states <- cbind(scenario1, timing_state_s1, recess_state_s1)
#### Trim data frame based on timing and recession rates ####
s1_states <- subset(s1_states, TIMING != "NA" & RECESSION != 1)
timing_subset <- s1_states[,14]
recess_subset <- s1_states[,15]

#### Allow dimension naming using the "bigmemory" package ####
options(bigmemory.allow.dimnames=TRUE)

#### Create empty matrix to fill with data based on the Q-bin ####
emptymatrix1 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q1_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q1_cells[,1]))))
emptymatrix2 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q2_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q2_cells[,1]))))
emptymatrix3 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q3_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q3_cells[,1]))))
emptymatrix4 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q4_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q4_cells[,1]))))
emptymatrix5 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q5_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q5_cells[,1]))))
emptymatrix6 <- big.matrix(nrow=nrow(s1_states), ncol=(nrow(q6_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q6_cells[,1]))))

# USE FOR WINDOWS #
# emptymatrix1 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q1_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q1_cells[,1]))))
# emptymatrix2 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q2_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q2_cells[,1]))))
# emptymatrix3 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q3_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q3_cells[,1]))))
# emptymatrix4 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q4_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q4_cells[,1]))))
# emptymatrix5 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q5_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q5_cells[,1]))))
# emptymatrix6 <- matrix(nrow=nrow(s1_states), ncol=(nrow(q6_cells)+2), dimnames=list(NULL, c("timing", "recess", as.character(q6_cells[,1]))))
...
q1_norm <- normalize(s1_states, q1_cell_probs)
q2_norm <- normalize(s1_states, q2_cell_probs)
q3_norm <- normalize(s1_states, q3_cell_probs)
q4_norm <- normalize(s1_states, q4_cell_probs)
q5_norm <- normalize(s1_states, q5_cell_probs)
q6_norm <- normalize(s1_states, q6_cell_probs)
...
if (exists("q6_prob_mn_norm")) q_all_mn_norm_s1 <- rbind(q1_prob_mn_norm, q2_prob_mn_norm, q3_prob_mn_norm, q4_prob_mn_norm, q5_prob_mn_norm, q6_prob_mn_norm)
if (!exists("q6_prob_mn_norm")) q_all_mn_norm_s1 <- rbind(q1_prob_mn_norm, q2_prob_mn_norm, q3_prob_mn_norm, q4_prob_mn_norm, q5_prob_mn_norm)

#### Write csv file output ####
write.csv(q1_prob_mn_norm, file="output/site1/q1_prob_mn_norm_s1.csv")
write.csv(q2_prob_mn_norm, file="output/site1/q2_prob_mn_norm_s1.csv")
write.csv(q3_prob_mn_norm, file="output/site1/q3_prob_mn_norm_s1.csv")
write.csv(q4_prob_mn_norm, file="output/site1/q4_prob_mn_norm_s1.csv")
write.csv(q5_prob_mn_norm, file="output/site1/q5_prob_mn_norm_s1.csv")
write.csv(q6_prob_mn_norm, file="output/site1/q6_prob_mn_norm_s1.csv")

# Site 1
write.csv(q_all_mn_norm_s1, file="output/site1/q_all_mn_norm_site1_s1.csv")
q_all_mn_norm_s12 <- read.csv("output/site1/q_all_mn_norm_site1_s1.csv")
colnames(q_all_mn_norm_s12) <- c("cell","mean_prob")
q_all_mn_norm_s12 <- q_all_mn_norm_s12[order(q_all_mn_norm_s12$cell),]
write.csv(q_all_mn_norm_s12, file="output/site1/q_all_mn_norm_site1_s1.csv", row.names=FALSE)

# Site 2
# write.csv(q_all_mn_norm_s1, file="output/site2/q_all_mn_norm_site2_s1.csv")
# q_all_mn_norm_s12 <- read.csv("output/site2/q_all_mn_norm_site2_s1.csv")
# colnames(q_all_mn_norm_s12) <- c("cell","mean_prob")
# q_all_mn_norm_s12 <- q_all_mn_norm_s12[order(q_all_mn_norm_s12$cell),]
# write.csv(q_all_mn_norm_s12, file="output/site2/q_all_mn_norm_site2_s1.csv", row.names=FALSE)

# Site 3
# write.csv(q_all_mn_norm_s1, file="output/site3/q_all_mn_norm_site3_s1.csv")
# q_all_mn_norm_s12 <- read.csv("output/site3/q_all_mn_norm_site3_s1.csv")
# colnames(q_all_mn_norm_s12) <- c("cell","mean_prob")
# q_all_mn_norm_s12 <- q_all_mn_norm_s12[order(q_all_mn_norm_s12$cell),]
# write.csv(q_all_mn_norm_s12, file="output/site3/q_all_mn_norm_site3_s1.csv", row.names=FALSE)

# Site 4
# write.csv(q_all_mn_norm_s1, file="output/site4/q_all_mn_norm_site4_s1.csv")
# q_all_mn_norm_s12 <- read.csv("output/site4/q_all_mn_norm_site4_s1.csv")
# colnames(q_all_mn_norm_s12) <- c("cell","mean_prob")
# q_all_mn_norm_s12 <- q_all_mn_norm_s12[order(q_all_mn_norm_s12$cell),]
# write.csv(q_all_mn_norm_s12, file="output/site4/q_all_mn_norm_site4_s1.csv", row.names=FALSE)

# Site 5
# write.csv(q_all_mn_norm_s1, file="output/site5/q_all_mn_norm_site5_s1.csv")
# q_all_mn_norm_s12 <- read.csv("output/site5/q_all_mn_norm_site5_s1.csv")
# colnames(q_all_mn_norm_s12) <- c("cell","mean_prob")
# q_all_mn_norm_s12 <- q_all_mn_norm_s12[order(q_all_mn_norm_s12$cell),]
# write.csv(q_all_mn_norm_s12, file="output/site5/q_all_mn_norm_site5_s1.csv", row.names=FALSE)